...
class excise_category(models.Model):
    ...
    def _calc_excise(self,product,quantity):
        ...
        for cat in product.excise_category_ids:
            cat_values = {
                'company_id' : cat.company_id,
                'currency_id' : cat.currency_id.id,
                'excise_category' : cat.id,
                'excise_rate' :cat.rate,
            }
            if cat.rate_per == 'hectoabv':
                cat_values['excise_amount_tax'] = alcohol_vol * cat.rate
            elif product.excise_category.rate_per == 'hectoprod':
                cat_values['excise_amount_tax'] = quantity * product._get_excise_volume() * cat.rate
